...
async function commitToGitHub(token, newData) {
  ...
  const res = await fetch(api, {
    ...
    headers,
    body: JSON.stringify({
      message: "content: update portfolio data via admin",
      content: encoded,
      sha: current.sha,
  ...
  return (
    <div className="space-y-2">
      <div className="flex gap-2">
        ...
        <button type="button" onClick={add} className="px-3 py-2 bg-bg border-2 border-border text-secondary hover:text-primary hover:border-accent transition-colors">
          <Plus size={14} />
        </button>
      </div>
      ...
      {data.projects.map((project, i) => (
        <div key={project.id} className="bg-surface2 border-2 border-border overflow-hidden">
          <div className="flex items-center gap-3 px-5 py-4 cursor-pointer" onClick={() => setExpanded(expanded === i ? null : i)}>
            ...
            <label className="flex items-center gap-2 font-mono text-xs text-faint cursor-pointer" onClick={e => e.stopPropagation()}>
              <input type="checkbox" checked={!!project.wide} onChange={e => updProject(i, "wide", e.target.checked)} className="accent-accent" /> Wide
            </label>
...
function ExperienceEditor({ data, onChange }) {
  ...
  const updItem = (i, key, val) => { const n = [...data.experience]; n[i] = { ...n[i], [key]: val }; upd(n) }
...
function EducationEditor({ data, onChange }) {
  ...
  const updItem = (i, key, val) => { const n = [...data.education]; n[i] = { ...n[i], [key]: val }; upd(n) }
...
const TAB_EDITORS = { Meta: MetaEditor, About: AboutEditor, Skills: SkillsEditor, Projects: ProjectsEditor, Experience: ExperienceEditor, Education: EducationEditor, Contact: ContactEditor }
...
export function Admin() {
  ...
  const save = async () => {
    // always update localStorage for instant preview
  ...
  const resetSection = () => {
    const keyMap = { meta: "meta", about: "about", skills: "skills", projects: "projects", experience: "experience", education: "education", contact: "contact" }
      ...
      <div className="border-b-2 border-border bg-surface px-6 py-4 flex items-center justify-between sticky top-0 z-10">
        <div className="flex items-center gap-3">
          ...
          {ghToken
            ...
            : <button onClick={() => setShowGHSetup(true)} className="flex items-center gap-1 font-mono text-xs text-faint border border-border px-2 py-0.5 hover:border-accent hover:text-accent transition-colors">
                <Settings size={10} /> Setup GitHub
              </button>
          }
        </div>
        <div className="flex items-center gap-4">
          {ghToken && (
            <button onClick={() => setShowGHSetup(true)} className="font-mono text-xs text-faint hover:text-secondary transition-colors">
              <Settings size={14} />
            </button>
          )}
          <Link to="/" className="flex items-center gap-1 font-mono text-xs text-secondary hover:text-primary transition-colors">
            View Site <ExternalLink size={12} />
          </Link>
          <button onClick={logout} className="flex items-center gap-1 font-mono text-xs text-faint hover:text-accent transition-colors">
      ...
      <div className="max-w-6xl mx-auto px-6 py-8 flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex lg:flex-col gap-2 overflow-x-auto lg:overflow-x-visible lg:w-44 shrink-0">
          {TABS.map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              className={`shrink-0 text-left px-4 py-2.5 font-mono text-sm transition-colors uppercase tracking-widest ${
                activeTab === tab ? "bg-accent text-bg" : "text-secondary hover:text-primary hover:bg-surface"
              }`}>
              {tab}
            </button>
          ))}
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center justify-between mt-4">
            <div>
              {!confirmReset
                ? <button onClick={() => setConfirmReset(true)} className="flex items-center gap-1 font-mono text-xs text-faint hover:text-accent transition-colors">
                    <RotateCcw size={12} /> Reset section to defaults
                  </button>
                : <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-3">
              ...
              {saveStatus === "success" && (
                <span className="flex items-center gap-1 font-mono text-xs text-green-400">
                  <CheckCircle2 size={12} /> Committed — rebuilding in ~2 min
                </span>
              )}
              <button onClick={save} disabled={saveStatus === "saving"}
                ...
                {saveStatus === "saving"
                  ...
                  : <><Save size={14} /> {ghToken ? "Save & Deploy" : "Save"}</>
                }
              </button>
            </div>
          </div>
          ...
          {!ghToken && (
            <div className="mt-4 border-2 border-dashed border-border p-4 flex items-center justify-between gap-4">
              <p className="font-mono text-xs text-faint">
                Changes save locally only. Connect GitHub to deploy to everyone.
              </p>
